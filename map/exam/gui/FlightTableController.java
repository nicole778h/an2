...
import com.example.exam.domain.DTOs.FlightDTO;
import com.example.exam.domain.Ticket;
import com.example.exam.observer.Observer;
...
//Verifică dacă câmpurile fromComboBox și
// toComboBox au fost selectate și dacă datePicker conține o dată validă.
//onvertște datele despre zboruri în obiecte FlightDTO și le adaugă în lista flights a controller-ului.
    public void searchClicked() {
        ...
        if (data.size() >= rowsPerPage) {
            flights.setAll(data.subList(0, rowsPerPage));
        } else {
            flights.setAll(data);
        }
